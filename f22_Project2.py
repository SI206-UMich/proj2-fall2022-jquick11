...
.
    ...
    """
    # opeing html file and parsing it BeautifulSoup
    ...
    for image in images:
        try:
            ...
            pass
    # creating a list of tuple with value (title, price, listing_id)
    list_of_tuples = []
    for i in range(len(titles)):
...
def get_detailed_listing_database(html_file):
    ...
    """
    # initializing an emtpy list which fill contain the detailed data
    listing_database = []
    # getting results from the get_listings_from_search_results() function
    listing = get_listings_from_search_results(html_file)
    for x in listing:  # iterating over those results
        # getting results from get_listing_information() function by passing listing id on every iteration
        listing_info = get_listing_information(x[2])
        record = x + listing_info  # combining the results
        listing_database.append(record)  # adding that record into the list
...
def write_csv(data, filename):
    ...
    """
    sorted_data = sorted(
        data, key=lambda t: t[1])  # sorting data based on its cost in ascending order
    out = open(filename, 'w', newline='')
    csv_out = csv.writer(out)
    csv_out.writerow(["Listing Title", "Cost", "Listing ID",
                     "Policy Number", "Place Type", "Number of Bedrooms"])
    csv_out.writerows(sorted_data)  # writing data to list
    out.close()
...
def check_policy_numbers(data):
    ...
    for x in data:
        policy_num = x[3]
        if policy_num == 'Pending' or policy_num == 'Exempt':  # skiping the pending or exempt cases
            continue
        # matching policy number format
        matched = re.findall(
            '20[0-9][0-9]-00[0-9][0-9][0-9][0-9]STR|STR-000[0-9][0-9][0-9][0-9]', policy_num)
        if len(matched) == 0:  # meaning they didnot match the format
